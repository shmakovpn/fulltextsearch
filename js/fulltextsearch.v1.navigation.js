...
var nav = {
		...
		fillDivResultExcepts: function (divResult, entry) {
			...
			if (entry.excerpts.length > 0) {
				//divResult.find('#extract').text(entry.excerpts[0]['excerpt']); // commented by shmakovpn because it shows only one fragment of 
				// highlight.content excerpts
				// 2019-10-28 shmakovpn
				// shows all excerpts of highlight.content
				height += 22;
				var divExtract = divResult.find('#extract');
				divExtract.html('');
				if(divExtract) {
					for(var i=0;i<entry.excerpts.length;i++) {
						divExtract.append('<div class="files_result_extract_item">'+entry.excerpts[i]['excerpt']+'</div>');
						height += 15;
                    }
				} // end shmakovpn
			} else {
			...
			// 2019-10-30 shmakovpn set height of .result_template
			divResult.css('height',height+'px');
		},
